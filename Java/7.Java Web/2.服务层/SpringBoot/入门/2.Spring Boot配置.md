...
SpringBoot使用一个全局的配置文件，配置文件名是固定的，但是他有两种配置形式；
...
配置文件的作用：使用了Spring Boot我们少写了非常多的配置，因为Spring Boot已经帮我们写了，配置文件就是修改Spring Boot帮我们写好的配置。
...
由于生产环境与开发环境毕竟还是有点不同的,所以很多时候开发环境的配置和生产环境的配置会有点区别,这时候我们就可以使用Profile了.
