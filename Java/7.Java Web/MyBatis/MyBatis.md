...
`MapperProxy`是一个`InvocationHandler`，也就是说实际的操作是由它来完成的。
...
我们可以看到在该接口中一共定义有三个方法，`intercept`、`plugin`和`setProperties`。`plugin`方法是拦截器用于封装目标对象的，通过该方法我们可以返回目标对象本身，也可以返回一个它的代理。当返回的是代理的时候我们可以对其中的方法进行拦截来调用`intercept`方法。
